...
type NicknameRules = {
    ...
    isNicknameOnlyEnglishAlphabetValidation:boolean
}
...
type PasswordRules = {
    isPasswordAtLeast8Characters:boolean,
    isPasswordHasAtLeast1SpecialCharacter:boolean,
    isPasswordNoForeignCharacters:boolean,
    isPasswordHasAtLeastTwoCapitalLetters:boolean,
}
...
const Form = () => {
    ...
    const [phonenumberInputValue,setPhonenumberPasswordInputValue] = useState<string>('');
    ...
    const [nicknameRules,setNicknameRules] = useState<NicknameRules>({
        isNicknameLengthValid:false,
        isNicknameOnlyEnglishAlphabetValidation:false
    })
    ...
    const [passwordRules,setPasswordRules] = useState<PasswordRules>({
       isPasswordAtLeast8Characters:false,
       isPasswordHasAtLeast1SpecialCharacter:false,
       isPasswordHasAtLeastTwoCapitalLetters:false,
       isPasswordNoForeignCharacters:false, 
    })

    const [confirmPasswordRules,setConfirmPasswordRules] = useState<ConfirmPasswordRules>({
        isConfirmPasswordEqualToPassword:false
    })

    const [phonenumberRules,setPhonenumberRules] = useState<PhonenumberRules>({
        isPhonenumber11Digits:false
    })
     ...
     function handleNicknameChange(value:string){
        ...
        setNicknameRules({
            ...nicknameRules,
            isNicknameLengthValid:checkForNicknameLengthValidation(value),
            isNicknameOnlyEnglishAlphabetValidation:checkForNicknameOnlyEnglishAlphabetValidation(value)

        })
    }

    function checkForNicknameLengthValidation(value:string){
        const nicknameLengthValidation = (value.length >= 3)
        return nicknameLengthValidation;
    }

    function checkForNicknameOnlyEnglishAlphabetValidation(value:string){
           const sanitizedInput = value.replace(/[^A-Za-z]/g,'')
           const nicknameOnlyEnglishAlphabetValidation = (value.length === sanitizedInput.length)
           return nicknameOnlyEnglishAlphabetValidation;
    }
    ...
    function handlePasswordChange(value:string){
        ...
        setPasswordRules({
            ...passwordRules,
            isPasswordAtLeast8Characters:checkForPasswordAtLeast8Characters(value),
            isPasswordHasAtLeast1SpecialCharacter:checkForPasswordHasAtLeast1SpecialCharacter(value),
            isPasswordHasAtLeastTwoCapitalLetters:checkForPasswordHasAtLeastTwoCapitalLetters(value),
            isPasswordNoForeignCharacters:checkForPasswordNoForeignCharacters(value)
        })
    }

    function checkForPasswordAtLeast8Characters(value:string){
        const isPasswordAtLeast8Characters = value.length >= 8;
        return isPasswordAtLeast8Characters; 
    }
    function checkForPasswordHasAtLeast1SpecialCharacter(value:string){
        const specialCharRegex = /[!@#$%^&*()_+{}\[\]:;<>,.?~\\/-]/;
        const isPasswordAtLeast1SpecialCharacters = specialCharRegex.test(value);
        return isPasswordAtLeast1SpecialCharacters;
    }
    function checkForPasswordHasAtLeastTwoCapitalLetters(value:string){
        const sanitizedInput= value.replace(/[^A-Z]/g,'')
        const doesPasswordHasAtLeastTwoCaptialLettersValidation = (sanitizedInput.length >= 2)
        return doesPasswordHasAtLeastTwoCaptialLettersValidation;
    }
    function checkForPasswordNoForeignCharacters(value:string){
        const sanitizedInput = value.replace(/[^A-Za-z0-9!@#$%^&*()_+{}\[\]:;<>,.?~\\/-]/g,'')
        const PasswordNoForeignCharactersValidation = (value.length === sanitizedInput.length)
        return PasswordNoForeignCharactersValidation;
    }
    function handleConfirmPasswordChange(value:string){
        setConfirmPasswordInputValue(value);
        setConfirmPasswordRules({
            ...confirmPasswordRules,
            isConfirmPasswordEqualToPassword:checkForConfirmPasswordEqualToPassword(value),
        })
    }
    function checkForConfirmPasswordEqualToPassword(value:string){
        const isConfirmPasswordEqualToPassword = (value === passwordInputValue)
        return isConfirmPasswordEqualToPassword;
    }
    function handlePhonenumberChange(value:string){
        ...
        setPhonenumberPasswordInputValue(phoneNumber)
        setPhonenumberRules({
            ...phonenumberRules,
            isPhonenumber11Digits:checkForPhonenumberHas11Digits(phoneNumber)
        })
    }     

    function checkForPhonenumberHas11Digits(value:string){
        const isPhonenumberHas11Digits = (value.length === 11)
        return isPhonenumberHas11Digits;
    }

    const areAllRulesValid = () => {
        // Check all the rules using logical AND (&&)
        return (
          nicknameRules.isNicknameLengthValid &&
          nicknameRules.isNicknameOnlyEnglishAlphabetValidation &&
          emailRules.isEmailValid &&
          passwordRules.isPasswordAtLeast8Characters &&
          passwordRules.isPasswordHasAtLeast1SpecialCharacter &&
          passwordRules.isPasswordHasAtLeastTwoCapitalLetters &&
          passwordRules.isPasswordNoForeignCharacters &&
          confirmPasswordRules.isConfirmPasswordEqualToPassword &&
          phonenumberRules.isPhonenumber11Digits
        );
      };
  return (
        !success ? 
        <form onSubmit={(e) => {
         ...
         className='flex flex-col gap-4'>
            <div className={`flex ${(nicknameInputValue.length === 0 || (nicknameRules.isNicknameLengthValid && nicknameRules.isNicknameOnlyEnglishAlphabetValidation) ? 'bg-WDS-sky-blue-75':'bg-WDS-red-75')} w-full px-5 rounded-md`}>
                <input 
                value={nicknameInputValue} 
                onChange={(e:ChangeEvent<HTMLInputElement>) => {handleNicknameChange(e.target.value)}} 
                className={` ${(nicknameRules.isNicknameLengthValid && nicknameRules.isNicknameOnlyEnglishAlphabetValidation ? 'text-WDS-sky-blue-500':'text-WDS-red-500')} placeholder-WDS-warm-gray-500 outline-none bg-transparent text-base font-medium py-3 w-full`}
                placeholder='Enter Nickname' 
                type="text" />
                {
                    nicknameInputValue.length > 0 ? 
                    <button onClick={(e:React.MouseEvent<HTMLButtonElement, MouseEvent>) => {e.preventDefault();setNicknameInputValue('')}} className=' flex justify-center items-center p-1'>
                ...
                <div className=' flex gap-1 flex-col items-start justify-start'>
                    {
                        ...
                        <span  className='text-sm text-WDS-green-500 font-semibold'>
                            Nickname should have at least 3 letters.
                        </span>
                        </div>
                        :
                        <div className='flex items-center gap-2'>
                    ...
                    }
                    {
                        ...
                        <svg   className='fill-WDS-green-500' xmlns="http://www.w3.org/2000/svg" height="18" viewBox="0 -960 960 960" width="18"><path d="M382-240 154-468l57-57 171 171 367-367 57 57-424 424Z"/></svg>
                        <span className='text-sm text-WDS-green-500 font-semibold'>
                            Nickname should have only english alphabet letters.
                        </span>
                        </div>
                        :
                        <div className='flex items-center gap-2'>               
            ...
            <div className={`${passwordInputValue.length === 0 || (passwordRules.isPasswordAtLeast8Characters && passwordRules.isPasswordHasAtLeast1SpecialCharacter && passwordRules.isPasswordHasAtLeastTwoCapitalLetters && passwordRules.isPasswordNoForeignCharacters) ?  'bg-WDS-sky-blue-75':'bg-WDS-red-75'} flex w-full px-5 rounded-md`}>
                <input
                 value={passwordInputValue}
                  onChange={(e:ChangeEvent<HTMLInputElement>) => {handlePasswordChange(e.target.value)}}
                  className={`placeholder-WDS-warm-gray-500 outline-none bg-transparent placeholder:text-base font-medium py-3 w-full ${passwordRules.isPasswordAtLeast8Characters && passwordRules.isPasswordHasAtLeast1SpecialCharacter && passwordRules.isPasswordHasAtLeastTwoCapitalLetters && passwordRules.isPasswordNoForeignCharacters ? 'text-WDS-sky-blue-500':'text-WDS-red-500'}`}
            ...
            </div>
            {
                ...
                <div className=' flex gap-1 flex-col items-start justify-start'>
                    ...
                    }
                    {
                        ...
                        <svg   className='fill-WDS-green-500' xmlns="http://www.w3.org/2000/svg" height="18" viewBox="0 -960 960 960" width="18"><path d="M382-240 154-468l57-57 171 171 367-367 57 57-424 424Z"/></svg>
                        <span  className='text-sm text-WDS-green-500 font-semibold'>
                            Password should have at least two capital letters.
                        </span>
                        </div>
                        :
                        <div className='flex items-center gap-2'>
                    ...
                    }
                    {
                        ...
                        <svg   className='fill-WDS-green-500' xmlns="http://www.w3.org/2000/svg" height="18" viewBox="0 -960 960 960" width="18"><path d="M382-240 154-468l57-57 171 171 367-367 57 57-424 424Z"/></svg>
                        <span  className='text-sm text-WDS-green-500 font-semibold'>
                            Password should have only english characters.                    
                        </span>
                        </div>
                        :
                        <div className='flex items-center gap-2'>
            ...
            <div className={`${confirmPasswordInputValue.length === 0 || confirmPasswordRules.isConfirmPasswordEqualToPassword ? 'bg-WDS-sky-blue-75':'bg-WDS-red-75'} flex w-full px-5 rounded-md`}>
                <input
                 value={confirmPasswordInputValue}
                  onChange={(e:ChangeEvent<HTMLInputElement>) => {handleConfirmPasswordChange(e.target.value)}}
                  className={`${confirmPasswordRules.isConfirmPasswordEqualToPassword ? 'text-WDS-sky-blue-500':'text-WDS-red-500' } placeholder-WDS-warm-gray-500 outline-none bg-transparent placeholder:text-base font-medium py-3 w-full`}
                  placeholder='Confirm Password'  
                  type={ isConfirmPasswordVisible ? 'text' : "password"} />
                {
                    confirmPasswordInputValue.length > 0 ? 
                    <button onClick={(e:React.MouseEvent<HTMLButtonElement, MouseEvent>) => {e.preventDefault();setIsConfirmPasswordVisible(!isConfirmPasswordVisible)}} className='flex justify-center items-center p-1'>
                         <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                         <path fillRule="evenodd" clipRule="evenodd" d="M12.5114 14.0663C10.9846 14.0663 9.74624 12.8286 9.74624 11.3011C9.74624 10.5272 10.0631 9.82955 10.5754 9.32693C10.7656 9.14031 10.7686 8.83483 10.5819 8.6446C10.3953 8.45438 10.0898 8.45145 9.89962 8.63807C9.21011 9.31449 8.78125 10.2573 8.78125 11.3011C8.78125 13.3617 10.4519 15.0312 12.5114 15.0312C13.5542 15.0312 14.498 14.6024 15.1744 13.9129C15.361 13.7227 15.3581 13.4172 15.1679 13.2306C14.9777 13.0439 14.6722 13.0469 14.4856 13.2371C13.983 13.7494 13.2845 14.0663 12.5114 14.0663Z" fill="#677185"/>
                         <path fillRule="evenodd" clipRule="evenodd" d="M4.86113 15.2958C6.41187 17.2002 8.87407 19.1562 12.4278 19.1562C14.838 19.1562 16.7565 18.2541 18.2279 17.0769C18.4338 16.9122 18.4662 16.6131 18.3003 16.4087C18.1344 16.2044 17.833 16.1723 17.6272 16.3369C16.2908 17.406 14.5783 18.2059 12.4278 18.2059C9.27031 18.2059 7.05482 16.478 5.60568 14.6984C4.88217 13.8099 4.35932 12.9191 4.01741 12.2496C3.84675 11.9154 3.72195 11.6379 3.64038 11.4455C3.62008 11.3976 3.60248 11.355 3.58752 11.3182C3.62404 11.2328 3.67542 11.1166 3.74195 10.9749C3.89472 10.6497 4.12678 10.1915 4.44151 9.66599C5.07304 8.61151 6.02699 7.30344 7.32593 6.24933C7.53061 6.08323 7.56089 5.78387 7.39356 5.58069C7.22623 5.37751 6.92466 5.34745 6.71998 5.51355C5.30516 6.66169 4.28386 8.06949 3.61858 9.18033C3.2849 9.73747 3.03816 10.2243 2.87423 10.5734C2.79221 10.748 2.73078 10.8885 2.68943 10.9864C2.66875 11.0353 2.65309 11.0737 2.64237 11.1003C2.63701 11.1137 2.63289 11.1241 2.62999 11.1315L2.62657 11.1402L2.62555 11.1429L2.62521 11.1438L2.62509 11.1441C2.62504 11.1442 2.62499 11.1443 3.07244 11.3132C2.62151 11.4726 2.62158 11.4729 2.62167 11.4731L2.62188 11.4737L2.6225 11.4754L2.62448 11.4808C2.62613 11.4853 2.62843 11.4916 2.63141 11.4995C2.63735 11.5154 2.64595 11.5381 2.65725 11.5672C2.67986 11.6253 2.71331 11.7088 2.75795 11.8141C2.84719 12.0246 2.9814 12.3227 3.16349 12.6793C3.52708 13.3912 4.08471 14.3424 4.86113 15.2958ZM3.07244 11.3132L2.62151 11.4726C2.58333 11.3663 2.58457 11.2499 2.62499 11.1443L3.07244 11.3132Z" fill="#677185"/>
                         <path fillRule="evenodd" clipRule="evenodd" d="M18.8437 15.599C19.0391 15.772 19.3392 15.7556 19.5141 15.5624C20.4324 14.5478 21.0912 13.4913 21.5202 12.6906C21.7352 12.2895 21.8935 11.9506 21.9987 11.7104C22.0513 11.5902 22.0907 11.4946 22.1172 11.428C22.1305 11.3948 22.1406 11.3687 22.1476 11.3506C22.1511 11.3415 22.1538 11.3343 22.1557 11.3292L22.158 11.3231L22.1587 11.3211L22.1589 11.3205L22.159 11.3202C22.1591 11.3201 22.1591 11.32 21.7128 11.16C22.1566 10.9932 22.1565 10.993 22.1564 10.9928L22.1555 10.9906L22.1535 10.9853C22.1517 10.9808 22.1493 10.9747 22.1462 10.9668C22.1399 10.9511 22.1308 10.9286 22.1189 10.8997C22.0951 10.8421 22.0599 10.7591 22.0131 10.6544C21.9195 10.4451 21.7795 10.1485 21.5911 9.79371C21.2149 9.08521 20.6431 8.13866 19.8603 7.18977C18.2995 5.29764 15.853 3.34375 12.4131 3.34375C10.8616 3.34375 9.50706 3.74313 8.34492 4.35717C8.11365 4.47936 8.02631 4.76388 8.14983 4.99266C8.27336 5.22143 8.56097 5.30784 8.79224 5.18564C9.83116 4.63672 11.0332 4.283 12.4131 4.283C15.4521 4.283 17.6554 6.00241 19.1247 7.78359C19.857 8.67135 20.3953 9.56145 20.7505 10.2304C20.9278 10.5644 21.0587 10.8418 21.1447 11.0342C21.1665 11.0829 21.1854 11.1262 21.2014 11.1635C21.1814 11.2118 21.1568 11.27 21.1274 11.337C21.0308 11.5578 20.8831 11.8742 20.6813 12.2507C20.277 13.0054 19.6596 13.9934 18.8067 14.9359C18.6318 15.1291 18.6484 15.426 18.8437 15.599ZM21.7128 11.16L22.1566 10.9932C22.1969 11.0983 22.1978 11.2143 22.1591 11.32L21.7128 11.16Z" fill="#677185"/>
            ...
            <div className={`${phonenumberInputValue.length === 0 || phonenumberRules.isPhonenumber11Digits ? 'bg-WDS-sky-blue-75':'bg-WDS-red-75'} flex w-full px-5 rounded-md`}>
                <input 
                value={phonenumberInputValue} 
                onChange={(e:ChangeEvent<HTMLInputElement>) => {handlePhonenumberChange(e.target.value)}} 
                className={`${phonenumberRules.isPhonenumber11Digits ? 'text-WDS-sky-blue-500':'text-WDS-red-500' } placeholder-WDS-warm-gray-500 outline-none bg-transparent text-base font-medium py-3 w-full`} 
                ...
                />
                {
                    ...
                    <button onClick={(e:React.MouseEvent<HTMLButtonElement, MouseEvent>) => {e.preventDefault();setPhonenumberPasswordInputValue('')}} className=' flex justify-center items-center p-1'>
                             <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect x="3.5" y="3.5" width="17.5" height="17.5" rx="8.75" stroke="#667085"/>
                            <path d="M14.7946 10.0021L10.0026 14.7941" stroke="#667085" strokeLinecap="round" strokeLinejoin="round"/>
                            <path d="M14.796 14.797L10 10" stroke="#667085" strokeLinecap="round" strokeLinejoin="round"/>
            ...
            <div className='flex w-full flex-col gap-12'>
                <input className='cursor-pointer flex w-full p-5 text-WDS-white-alpha-default tracking-widest bg-WDS-sky-blue-400 font-bold rounded-md justify-center items-center shadow-md shadow-WDS-sky-blue-20 disabled:opacity-20 transition-all disabled:pointer-events-none' 
                value='Register NOW!' 
                type="submit"
        ...
        :     
        <div className='flex flex-col items-center self-center justify-center gap-4'>
            <h2 className=' font-black text-4xl'>
                Success!
            </h2>
            <p className=' text-WDS-warm-gray-600'>
                {success}
            </p>
            <svg className='flex m-12 w-32 h-32 fill-WDS-green-400' xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24"><path d="M480-80q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q65 0 123 19t107 53l-58 59q-38-24-81-37.5T480-800q-133 0-226.5 93.5T160-480q0 133 93.5 226.5T480-160q133 0 226.5-93.5T800-480q0-18-2-36t-6-35l65-65q11 32 17 66t6 70q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Zm-56-216L254-466l56-56 114 114 400-401 56 56-456 457Z"/></svg>
            <Link className='px-12 py-3 text-WDS-white-alpha-default font-medium text-base bg-WDS-green-400 rounded-full flex' href={'/login'}>
